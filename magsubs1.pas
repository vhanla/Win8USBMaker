...
WARNING - Delphi 6 and later only, supports Delphi 2009 and later with Unicode
...
begin
    ...
    Temp := '';
    for I := 1 to L2 do
         Temp := AnsiString (FormatSettings.ThousandSeparator) + Copy (S, LS - 3 * I + 1, 3) + Temp;  // 7 Aug 2010
    Result := Copy (S, N, (LS - 1) mod 3 + 1) + Temp;
    if N > 1 then Result := '-' + Result ;
end;
...
begin
    ...
    result := IntToStr (day) + result + space +
            FormatSettings.LongMonthNames [month] + space + IntToStr (year) ;
end ;
...
begin
    ...
    result := IntToStr (hours) + string (FormatSettings.TimeSeparator) + info ;  // 7 Aug 2010
end ;
